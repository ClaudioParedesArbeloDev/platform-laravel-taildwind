                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-text-500 text-background-500 uppercase text-sm">
                            ...
                            <th class="py-3 px-4 text-left"></th> <!-- columna vacía para Delete -->
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($classes as $class)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <form action="{{ route('classes.update', $class->id) }}" method="POST">
                                    ...
                                    <td class="py-3 px-4">
                                        <input type="text" name="powerpoint" 
                                               value="{{ old('powerpoint', $class->powerpoint) }}"
                                               class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        @error('powerpoint')
                                            <span class="text-red-500 text-sm">{{ $message }}</span>
                                        @enderror
                                    ...
                                    <td class="py-3 px-4">
                                        <input type="text" name="meet_link" 
                                               value="{{ old('meet_link', $class->meet_link) }}"
                                               class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        @error('meet_link')
                                            <span class="text-red-500 text-sm">{{ $message }}</span>
                                        @enderror
                                ...
                                <!-- Formulario de ELIMINACIÓN (separado, sin anidar) -->
                                <td class="py-3 px-4">
                                    <form action="{{ route('classes.destroy', $class->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                                class="text-red-600 hover:text-red-800 font-medium bg-transparent border-0 cursor-pointer">
                                            {{ __('Delete') }}
                                        </button>
                                    </form>
                                </td>
